...
async function syncRouteExtras() {
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 5000);
    const mr = await fetch(API_BASE + '/api/maintenance', { signal: ctrl.signal, credentials: 'include' });
    clearTimeout(t);
    if (mr.ok) {
...
syncFromApi(updateAll);
void syncRouteExtras().catch(() => {});
